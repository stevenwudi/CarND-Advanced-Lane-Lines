...
class CameraCal:
    # initialize - either go through and calculate the camera calibration if no pickle file exists
    ...
    def __init__(self, calibration_dir, pickle_file):
        ...
        if not os.path.isfile(pickle_file):
            ...
            print('Performing camara calibrations against chessboard images: "./{}/calibration*.jpg"...'.format(calibration_dir))
            ...
            for idx, fname in enumerate(images):
                ...
                if ret:
                    ...
                    ret, self.mtx, self.dist, rvecs, tvecs = cv2.calibrateCamera(objpoints, imgpoints, self.img_size,
                                                                                 None, None)

            # done and found all chessboard corners.
            # now time to save the results into a pickle file for later retrieval without additional calculations.
            try:
                with open(pickle_file, 'w+b') as pfile1:
                    print('Saving data to pickle file: {} ...'.format(pickle_file))
                    pickle.dump({'img_size': self.img_size, 'mtx': self.mtx, 'dist': self.dist}, pfile1, pickle.HIGHEST_PROTOCOL)
                    print("Camera Calibration Data saved to", pickle_file)
            except Exception as e:
                print('Unable to save data to', pickle_file, ':', e)
                raise

        # previously saved pickle file of the distortion correction data has been found
        # go ahead and revive it.
            ...
            try:
                ...
                print('Unable to restore camera calibration data from', pickle_file, ':', e)
                raise

    # Get the camera calibration result that the rest of the pipeline wants
    def get(self):
