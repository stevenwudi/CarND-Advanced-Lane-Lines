...
class ImageFilters:
    ...
    def __init__(self, camCal, debug=False):
        ...
        self.curRoadEdgeProjected = np.zeros((self.y, self.x, 3), dtype=np.uint8)

        # current Edge (Right Only)
        self.curRoadREdge = np.zeros((self.mid, self.x), dtype=np.uint8)
        self.curRoadREdgeProjected = np.zeros((self.y, self.x, 3), dtype=np.uint8)

        # current Edge (Left Only)
        self.curRoadLEdge = np.zeros((self.mid, self.x), dtype=np.uint8)
    ...
    def canny(self, img, low_threshold, high_threshold, kernel_size):
        img = self.image_only_yellow_white(img)
    ...
    def edges(self, side=3):
        ...
        if side == 1:
            img[self.mid:self.y, :, :] = np.dstack((self.curRoadLEdge, self.curRoadLEdge, self.curRoadLEdge))
        elif side == 2:
            img[self.mid:self.y, :, :] = np.dstack((self.curRoadREdge, self.curRoadREdge, self.curRoadREdge))
        else:
    ...
    # function to attempt to balance the image exposure for easier lane line detection
    def balanceEx(self):
    ...
    def applyFilter1(self, side=3):
        ...
        combined[((gradx > 0) | (grady > 0) | ((magch > 0) & (dirch > 0)) | (sch > 0)) & (shadow == 0) & (
        rEdgeDetect > 0)] = 35
        if (side & 1) == 1:
            self.curRoadLEdge = np.copy(combined)
        if (side & 2) == 2:
            self.curRoadREdge = np.copy(combined)
        if side == 3:
    ...
    def applyFilter4(self, side=3):
        ...
        # build the combination
        combined = np.zeros_like(dirch).astype(np.uint8)
        combined[((magch > 0) & (dirch > 0)) | ((rEdgeDetect > 192) & (rEdgeDetect < 200) & (magch > 0))] = 35
        if (side & 1) == 1:
            self.curRoadLEdge = np.copy(combined)
        if (side & 2) == 2:
            self.curRoadREdge = np.copy(combined)
    ...
    def applyFilter5(self, side=3):
        ...
        # create the Red filter
        rEdgeDetect = img[:, :, 0] / 4
        # rEdgeDetect = ((self.curRoadRGB[:,:,0]/4)).astype(np.uint8)
        rEdgeDetect = 255 - rEdgeDetect
        rEdgeDetect[(rEdgeDetect > 220)] = 0
    ...
    def setProjection(self, projected, side=3):
        if (side & 1) == 1:
            self.curRoadLEdgeProjected = np.copy(projected)
        if (side & 2) == 2:
            self.curRoadREdgeProjected = np.copy(projected)
        if side == 3:
    ...
    def getProjection(self, side=3):
        if side == 1:
            return self.curRoadLEdgeProjected
        if side == 2:
            return self.curRoadREdgeProjected
